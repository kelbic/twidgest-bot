...
# --------------------------------------------------------------------------- #
# Пер-канальные капы (этап B, перед per-channel биллингом 999⭐/канал).
# Цифры временные — уточняются по cost-totals после недели данных.
# --------------------------------------------------------------------------- #

# Источников на один канал. Честный пользователь не упрётся: AI-онбординг
# после превалидации отдаёт 6-9. Кап — против абьюза по API-кредитам.
MAX_SOURCES_PER_CHANNEL = 15

# LLM-оценок (ranker + rewrite) на канал в сутки. По cost-totals нормальный
# hybrid-канал тратит ~50-100; кап ловит хвостовые сценарии single-режима.
DAILY_EVAL_BUDGET = 150
