...
# STALE_CAP_ACTIVE=2 → потолок 2ч (1800*2^2=7200s) для активных. Это ловит источники
# с паузами 2-3ч между постами (опрос раз в 2ч вместо раздувания до 6ч), не раздувая
# запросы к мёртвым (у них последний твит дни назад → кэп не применяется → backoff до
# 6ч как раньше). 2ч (не 1ч) выбрано ради экономии запросов: 12 опросов/день на активный
# источник вместо 24, достаточно ловить посты с паузами 2-3ч.
...
class TwitterCache:
    ...
    async def get_tweets(self, username: str, limit: int = 20) -> list[Tweet]:
        ...
        async with lock:
            ...
            # аккаунты (последний твит дни назад) не подпадают — у них backoff
            # растёт как раньше (экономия запросов сохранена).
            if tweets:
