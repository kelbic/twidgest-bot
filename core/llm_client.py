...
class OpenRouterClient:
    ...
    async def rewrite_tweet(
        ...
        Args:
            niche: код ниши (general, tech_ai, longevity, и т.д.)
            filter_mode: 'strict' (только факты) или 'loose' (новости/реакции).
                Backwards compat: news/community/entertainment маппятся.
        """
        user_prompt = (
            f"Автор: @{author}\n"
            f"URL: {tweet_url}\n\n"
        ...
        system_prompt = build_single_prompt(niche, filter_mode)
    ...
    async def suggest_sources(
        ...
        user = (
            ...
            "ВАЖНО: нужны авторы, которые пишут ЧАСТО (несколько раз в неделю "
            "и чаще) и собирают заметный отклик (лайки/репосты). Известный, но "
            "редко пишущий аккаунт НЕ подходит — канал с такими источниками "
            "молчит. Приоритет: активные новостники, аналитики, инсайдеры по "
            "теме, а не «звёзды» с редкими твитами.\n\n"
