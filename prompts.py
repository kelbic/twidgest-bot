...
# Слой A — юр.риски РФ. Отключается ПЕР-КАНАЛЬНО (/setlegal) под явную
# ответственность владельца; факт отказа фиксируется в БД (legal_optout_at).
SAFETY_LEGAL_RF = """\
...
def safety_rules(legal_rf: bool = True) -> str:
    """Защитные правила. Слой B+C (наркотики, дозировки) — всегда.
    Слой A (юр.риски РФ) — если legal_rf (канальная настройка, default ON)."""
    mid = SAFETY_LEGAL_RF + "\n" if legal_rf else ""
    return _SAFETY_HEAD + mid + _SAFETY_CORE


# Обратная совместимость: полный набор (слой A включён)
BASE_SAFETY = safety_rules(True)
...
_UNFILTERED_RULES = """D) ФИЛЬТР ЦЕННОСТИ:

Публикуй всё без исключений. Raw-канал без фильтрации по ценности.

НИКОГДА НЕ ВОЗВРАЩАЙ SKIP по причинам ценности контента.
Публикуй любой твит: мнения, реакции, анонсы, короткие фразы — всё.

Единственное исключение — BASE_SAFETY (юр. риски, наркотики, дозировки).
Эти правила действуют всегда и не отменяются этим фильтром.
"""
...
def build_single_prompt(niche_code: str = "general", filter_mode: str = "strict",
                        legal_rf: bool = True) -> str:
...
def build_vk_prompt(niche_code: str = "general", filter_mode: str = "strict",
                    legal_rf: bool = True) -> str:
...
FILTER_MODES: dict[str, FilterMode] = {
    ...
    "unfiltered": FilterMode(
        code="unfiltered",
        name="Без фильтра",
        emoji="⚡",
        description="Публикует всё без исключений (кроме юр. рисков). Используйте на свой страх и риск.",
    ),
}
...
def build_unfiltered_prompt(niche_code: str = "general", legal_rf: bool = True) -> str:
...
def build_unfiltered_digest_prompt(niche_code: str = "general", legal_rf: bool = True) -> str:
...
def build_digest_prompt(niche_code: str = "general", legal_rf: bool = True) -> str:
    """Собирает system prompt для дайджеста."""
    niche = get_niche(niche_code)

    return f"""Ты — ведущий редактор русскоязычного Telegram-канала {niche.name}.
Тема канала: {niche.topic}.

Тебе дан список твитов за последний период. Твоя задача — НЕ перевести их по
очереди, а написать СВЯЗНЫЙ обзор «что важного произошло в теме за период»:
3-6 тем, каждая объяснена так, чтобы читатель понял, ПОЧЕМУ это важно и как
связано с остальным. Это разница между лентой обрывков и текстом, который
хочется дочитать.

{safety_rules(legal_rf)}

КАК ПИСАТЬ КАЖДУЮ ТЕМУ:
- Начни с сути: что произошло и почему это значимо (не пересказ твита, а его смысл
  для того, кто следит за темой).
- Сохрани КОНКРЕТИКУ из твита: числа, имена, факты, даты. Без них обзор
  превращается в воду — этого избегай любой ценой.
- Где темы связаны (один тренд, противоположные ходы, причина-следствие) —
  покажи связь явно. Где не связаны — просто следующий абзац.
- Тон: живой и осмысленный, но БЕЗ преувеличений, кликбейта и выдуманных оценок.
  Угол = объяснение значимости фактами, а не накрученные эмоции.

ЧЕГО НЕ ДЕЛАТЬ:
- Не пиши «сегодня насыщенный день» и пустые вводные — сразу к делу.
- Не выдумывай факты, числа и связи, которых нет в твитах.
- Не оставляй пункт голым переводом твита без контекста значимости.

ФОРМАТ:
🌐 <b>{niche.name}: главное за период</b>
<i>Подзаголовок одной фразой — общий смысл подборки</i>

<b>Заголовок темы 1</b>
Связный абзац: суть, почему важно, конкретика с числами/именами. <a href="URL">→ источник</a>
...
ПРАВИЛА:
- 3-6 тем
- Каждая тема 250-450 знаков, со ссылкой на источник
- Без рекламы и призывов
"""
