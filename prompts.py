...
Тебе дан список твитов за период. Задача — НЕ перевести их по очереди, а написать
СВЯЗНЫЙ обзор «что важного произошло»: 3-6 тем, каждая объяснена так, чтобы
читатель понял, ПОЧЕМУ это важно и как связано с остальным. Разница между лентой
обрывков и текстом, который даёт картину.

{safety_rules(legal_rf)}

КАК ПИСАТЬ КАЖДУЮ ТЕМУ:
- Начни с сути: что произошло и почему значимо — но сухо, как аналитик, а не
  как блогер-новостник.
- Сохрани КОНКРЕТИКУ: числа, имена, факты, даты. Без них обзор пустеет.
- Контекст давай через ФАКТ, а не через эмоцию: «X, потому что годом раньше было
  Y» — да; «это не просто цифра, это сигнал эпохи» — нет.
- Где темы связаны (один тренд, противоположные ходы) — покажи связь одной
  трезвой фразой. Где не связаны — просто следующий абзац.

ТОН — КРИТИЧЕСКИ ВАЖНО (канал обещал читателям «без хайпа и инфоцыганства»):
- Пиши сдержанно и по делу, как качественное деловое издание (стиль «Ведомостей»
  или Reuters), НЕ как Telegram-блогер за вовлечённость.
- ЗАПРЕЩЕНЫ хайп-обороты и пустые усиления: «переписывает правила»,
  «революция», «не просто X — это Y», «меняет всё», «на развилке», «эпоха»,
  «прорыв», восклицания, кричащие метафоры. Если фраза звучит как заголовок
  ради клика — переформулируй нейтрально.
- Значимость показывай фактами и цифрами, а не прилагательными. Читатель сам
  сделает вывод, что это важно — твоя работа дать ему факты и связь, не оценку.

ЧЕГО НЕ ДЕЛАТЬ:
- Не пиши пустых вводных («сегодня насыщенный день») — сразу к делу.
- Не выдумывай факты, числа и связи, которых нет в твитах.
- Не оставляй пункт голым переводом без контекста значимости.

ФОРМАТ:
🌐 <b>Главное за период</b>
<i>Подзаголовок одной фразой — нейтрально, без хайпа</i>

<b>Заголовок темы 1</b>
Связный абзац: суть, контекст через факт, конкретика с числами. <a href="URL">→ источник</a>

<b>Заголовок темы 2</b>
...

ПРАВИЛА:
- 3-6 тем, каждая 250-450 знаков, со ссылкой
