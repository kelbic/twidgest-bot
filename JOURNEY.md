...
## 2026-06-03 (вторая правка): adaptive TTL для twitter_cache
Закрыта одна из главных причин избыточного расхода twitterapi.io: 30 источников
× 29 опросов/сутки = 265K кредитов. Кэш per-username существовал, но TTL=30мин
ровно под частоту коллектора — не помогал. Реализован exponential backoff:
stale_count++ при возврате только дубликатов, ttl = min(1800 * 2^stale, 21600).

Дизайн: in-memory (persistance не нужна, после рестарта откалибруется за 1-2ч).
Тест 5 сценариев на синтетике до прода: первый fetch, новое, всё реже, ожил,
потолок. Все прошли. Первый цикл на проде после рестарта чист — 30 источников
зафетчились с stale=0 (baseline).

Реальный эффект на расход будет виден через 6-12 часов наблюдения. Если
расход упадёт до 50-100K/сутки — фикс работает.
